...
const Dashboard = () => {
  ...
  const [sortField, setSortField] = useState<keyof Guest>("nome_completo");
  ...
  const calculateAgeGroups = (guestsList: Guest[]) => {
    ...
    guestsList.forEach((guest) => {
      ...
      else if (age <= 25) groups[1].count++;
      else if (age <= 35) groups[2].count++;
      else if (age <= 50) groups[3].count++;
      else groups[4].count++;
    });
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-secondary/30 to-background">
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card className="shadow-card hover:shadow-glow transition-all duration-300 border-primary/20 md:col-span-2">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
        ...
        <Card className="shadow-elegant border-primary/20">
          <CardHeader>
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
              <CardTitle className="text-2xl">Lista de Convidados</CardTitle>
                  ...
                  <TableBody>
                    {filteredGuests.length === 0 ? (
                      ...
                      filteredGuests.map((guest) => (
                        <TableRow key={guest.id} className="hover:bg-secondary/30">
                          <TableCell className="font-medium">
                            {guest.nome_completo}
                          </TableCell>
                          <TableCell>{guest.idade}</TableCell>
